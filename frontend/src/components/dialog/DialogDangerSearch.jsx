...
const DialogDangerSearch = ({ toggleDialog }) => {
  ...
  const handleSubmit = () => {
    ...
    const layers = typhoons.filter((row) => {
      if (indexes.indexOf(row.typhoon_id) !== -1) {
        return true;
      } else {
        return false;
      }
    ...
    // dispatch an action to select typhoon layers
  ...
  // cancel selection
  const handleCancel = () => {
    // clear all typhoons search result
    dispatch({ type: 'CLEAR_ALL_TYPHOONS' });
  ...
  const handleSelectRow = (event, id) => {
    ...
    let newSelected = [];
    if (selectedIndex === -1) {
      // if the id is not in the list, then add to the new list
