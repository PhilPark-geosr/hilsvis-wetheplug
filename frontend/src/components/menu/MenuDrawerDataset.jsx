...
import { AddRounded, AutorenewRounded, DeleteRounded, InfoRounded } from '@mui/icons-material';
...
const MenuDrawerDataset = () => {
  ...
  // deselect selected dataset
  const handleDeselectDataset = () => {
    // dispatch an action to deselect the dataset
  ...
  return (
    <React.Fragment>
      {/* 데이터셋 추가 */}
      <Box sx={{ ...sxBox }}>
        <Typography sx={{ my: '10px' }}>모델 데이터셋</Typography>
        {dataset === null && (
          <Button
            variant='contained'
            size='small'
      ...
      {/* 데이터셋 검색 다이얼로그 */}
      {openSearchDialog && <DialogDatasetSearch toggleSearchDialog={handleToggleSearchDialog} />}
      ...
      {dataset !== null && (
        <React.Fragment>
          <List sx={{ ...sxList }}>
            ...
            >
              <ListItemButton onClick={handleToggleInfoDialog}>
                <ListItemText
                ...
                <IconButton size='small' onClick={handleToggleInfoDialog}>
                  <InfoRounded />
                </IconButton>
              </ListItemButton>
            </ListItem>
          ...
          {/* 데이터셋 정보 다이얼로그 */}
          {openInfoDialog && <DialogDatasetTable toggleInfoDialog={handleToggleInfoDialog} />}
